...
/*
In this version of strassen, we do not allocate 17 matrices for the addition
& multiplication. We allocate a matrix for the addition and release it once its purpose is over.
For multiplication, we use the resulting matrix C to store 4 multiplication results.
We use 3 matrcies for storing the remaining 3 more multiplications. Hence for one call of strassen
there are only 5 matrices allocated at a time rather than the 17 matrcies. 
*/
                    ...
                    const size_t n)
{
    ...
    strassen_aux(P1, A, (const float* const *const)S1,
                    0, 0,
                    A_f_row, A_f_col,
                    0, 0,
                    n2);
    ...
    //P2 = S2 * B22
    strassen_aux(C, (const float* const *const)S2, B,
                    C_f_row, C_f_col + n2,
                    0, 0,
                    B_f_row + n2, B_f_col + n2,
                    n2);
    ...
    strassen_aux(P3, (const float* const *const)S3, B,
                    0, 0,
                    0, 0,
                    B_f_row, B_f_col,
                    n2);  
    ...
    sub_matrix_blocks(S4, B, B, 
                            ...
                            n2, n2);
    // P4 = A22 * S4
    strassen_aux(C, A, (const float* const *const)S4,
                    C_f_row + n2, C_f_col,
                    A_f_row + n2, A_f_col + n2,
                    0, 0,
                    n2);
    ...
    // P5 = S5 * S6
     strassen_aux(C, (const float* const *const)S5, 
    ...
    //P6 = S7 * S8  
    strassen_aux(C, (const float* const *const)S7, 
    ...
    strassen_aux(P7, (const float* const *const)S9, 
