...
int main(int argc, char *argv[])
{
    ...
    for (i=2; (1<<i)<=MAX_SIZE_ALL; i++) {
        ...
        test_and_print(heap_sort, A_sorted,
                       A_size, sizeof(int),
                       leq_int, NUM_OF_REPETITIONS);
        test_and_print(heap_sort, A_rev_sorted,
                       A_size, sizeof(int),
                       leq_int, NUM_OF_REPETITIONS);
    }
    printf("\n\n\n");

    printf("Size\tBubble Sort\t          \t        \n");
    printf("    \t(Random Case)\t(Best Case)\t(Worst Case)");
    for (i=2; (1<<i)<=MAX_SIZE_ALL; i++) {
        const unsigned int A_size=1<<i;
        printf("\n2^%d",i);
        test_and_print(bubble_sort, A,
                       A_size, sizeof(int),
                       leq_int, NUM_OF_REPETITIONS);
        test_and_print(bubble_sort, A_sorted,
                       A_size, sizeof(int),
                       leq_int, NUM_OF_REPETITIONS);
        test_and_print(bubble_sort, A_rev_sorted,
                       A_size, sizeof(int),
                       leq_int, NUM_OF_REPETITIONS);
    }
    printf("\n\n\n");
    ...
    for (i=2; (1<<i)<=MAX_SIZE_ALL; i++) {
        ...
        test_and_print(quick_sort, A_sorted,
                       A_size, sizeof(int),
                       leq_int, NUM_OF_REPETITIONS);
        test_and_print(quick_sort, A_rev_sorted,
                       A_size, sizeof(int),
                       leq_int, NUM_OF_REPETITIONS);
    }
    printf("\n\n\n"); 
