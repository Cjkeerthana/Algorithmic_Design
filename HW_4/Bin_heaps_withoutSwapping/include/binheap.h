...
typedef struct {
    ...
    total_order_type leq; // the total order of the heap
    void *max_order_value; // the maximum value stored in the heap acc. to the order
    unsigned int *key_position; // To store the position of the key in the array A
    unsigned int *node_position; // To store the position of the node in the key position array
    size_t key_pos_size; // size of the key_pos array type
} binheap_type;
